...
import React, { useState, useEffect, useRef, useCallback } from 'react';
import { FiFilter, FiChevronDown, FiCheck, FiX, FiRefreshCw } from 'react-icons/fi';
import { parseISO, isValid, subDays } from 'date-fns'; // For robust date handling
...
const parseDate = (dateStr) => {
  ...
  try {
    const date = parseISO(dateStr); // Handles ISO 8601 (e.g., 2025-05-27T12:00:00Z)
    return isValid(date) ? date : null;
  } catch {
    // Fallback for other formats (e.g., MM/DD/YYYY, DD-MM-YYYY)
...
const applyAdvancedFilters = (currentFilters = {}, jobData = []) => {
  ...
  // Job Type filtering
  if (currentFilters.jobtype) {
    filteredResults = filteredResults.filter((job) => job.jobtype === currentFilters.jobtype);
  }

  // Experience filtering
  if (currentFilters.experience) {
    filteredResults = filteredResults.filter((job) => job.experience === currentFilters.experience);
  }
  ...
  if (currentFilters.salary && currentFilters.salary > 0) {
    const isHourly = currentFilters.salaryType === 'hourly';
    const minSalary = Number(currentFilters.salary);
    const minSalaryUSD = isHourly ? minSalary * 40 * 52 : minSalary * 1000;
    ...
    filteredResults = filteredResults.filter((job) => {
      ...
      console.log(
        `Job "${job.title || 'Unknown'}": USD=${jobSalaryUSD}, Raw LPA=${job.lpa}, Filter USD=${minSalaryUSD}, Pass=${
          jobSalaryUSD >= minSalaryUSD
        }`
      );
      return jobSalaryUSD >= minSalaryUSD;
    });
...
function QuickFilterDropdowns({
  ...
  const [salaryValue, setSalaryValue] = useState(filters.salary || 0);
  const [tempSalaryValue, setTempSalaryValue] = useState(salaryValue);
  const [salaryType, setSalaryType] = useState(filters.salaryType || 'annual');
  const [isDragging, setIsDragging] = useState(false);
  const dropdownContainerRef = useRef(null);
  const [isMobile, setIsMobile] = useState(window.innerWidth < 768);
  ...
  const filterOptions = [
    {
      key: 'jobmode',
      label: 'Job Mode',
      tooltip: 'Filter by work arrangements',
      options: [
    ...
    },
    {
      key: 'jobtype',
      label: 'Job Type',
      tooltip: 'Filter by employment type',
      options: [
    ...
    },
    {
      key: 'experience',
      label: 'Experience',
      tooltip: 'Filter by experience level',
      options: [
    ...
    },
    {
      key: 'location',
      label: 'Location',
      tooltip: 'Filter by job location',
      options: [
  ...
  const isFilterActive = (key) =>
    key === 'salary' ? filters[key] > 0 : filters[key] !== '' && filters[key] !== undefined;
  ...
  const formatSalary = (value) =>
    salaryType === 'hourly' ? `$${Number(value).toFixed(1)}/Hour` : `$${Number(value).toLocaleString('en-US')}K`;
