...
const SubscriptionPlanManager = () => {
  ...
  // Show error/success messages with SweetAlert2
  useEffect(() => {
    if (error) {
      Swal.fire({
        icon: 'error',
  ...
  const fetchPlans = useCallback(async () => {
    ...
    try {
      ...
      const normalizedPlans = response.data.map((plan) => ({
        ...plan,
        name: Object.values(ALLOWED_PLAN_NAMES).includes(plan.name.toLowerCase())
          ? plan.name.toLowerCase()
          : ALLOWED_PLAN_NAMES.BASIC,
  ...
  const openModal = (plan = null) => {
    setState((prev) => ({
      ...
      formData: plan
        ? {
            name: Object.values(ALLOWED_PLAN_NAMES).includes(plan.name)
              ? plan.name
              : ALLOWED_PLAN_NAMES.BASIC,
  ...
  const validateForm = () => {
    ...
    if (!Object.values(ALLOWED_PLAN_NAMES).includes(name))
      return `Plan name must be one of: ${Object.values(ALLOWED_PLAN_NAMES).join(', ')}`;
  ...
  const handleDelete = async (planId) => {
    const result = await Swal.fire({
      title: 'Are you sure?',
      text: 'This action will delete the subscription plan. This cannot be undone.',
      icon: 'warning',
    ...
    if (result.isConfirmed) {
      ...
      try {
        ...
        if (err.response?.status === 401) {
          ...
          errorMessage = err.response.data.error; // Handles "Cannot delete plan with X active subscriptions"
        }
        setState((prev) => ({ ...prev, error: errorMessage }));
    ...
    <>
      ...
      <div className="table-responsive" style={{ marginLeft: '270px', marginTop: '60px', marginBottom: '90px' }}>
        <div className="button-container">
          <button onClick={() => openModal()} className="add-plan-button" aria-label="Add new plan">
            Add New Plan
          </button>
        </div>
        ...
        <table className="plans-table">
          ...
          <tbody>
            {loading && !plans.length ? (
              ...
              plans.map((plan) => (
                <tr key={plan.id}>
                  <td data-label="Name">{plan.name.charAt(0).toUpperCase() + plan.name.slice(1)}</td>
                  ...
                  <td data-label="Actions" className="actions-cell">
                    ...
                    >
                      {deletingPlanId === plan.id ? <span className="mini-spinner"></span> : 'Delete'}
                    </button>
                  </td>
                </tr>
              ))
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="modal-form">
              ...
              <div className="form-note">
                <p>
                  <strong>Note:</strong> Plan duration is fixed at 30 days.
                </p>
              </div>
              <div className="form-actions">
                ...
                <button type="button" onClick={closeModal} className="cancel-button" disabled={loading}>
                  Cancel
                </button>
              </div>
            </form>
