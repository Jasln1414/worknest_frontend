...
import '../../assets/Employer/EmployerList.css';
...
function EmployerList() {
    ...
    const handleApproval = async (id, action) => {
        setIsLoading(true); // Show loading during approval/rejection
        try {
    ...
    return (
        <div className="el-content-wrapper">
            ...
            <div className="el-main-content">
                ...
                {isLoading ? (
                    <div className="el-loading">Loading...</div>
                ) : (
                            ...
                            <tbody>
                                {filteredEmployers.map((employer, index) => (
                                    <tr key={index} className="el-table-row">
                                        <td className="el-table-cell" data-label="Profile Pic">
                                        ...
                                        <td className="el-table-cell" data-label="Name">
                                            <Link to={`/admin/edetails/${employer.id}`} className="el-link-wrapper">
                                                {employer.user_name || 'N/A'}
                                            </Link>
                                        </td>
                                        <td className="el-table-cell" data-label="Email">
                                            <Link to={`/admin/edetails/${employer.id}`} className="el-link-wrapper">
                                                {employer.email || 'N/A'}
                                            </Link>
                                        </td>
                                        <td className="el-table-cell" data-label="Phone">
                                            <Link to={`/admin/edetails/${employer.id}`} className="el-link-wrapper">
                                                {employer.phone || 'N/A'}
                                            </Link>
                                        </td>
                                        <td className="el-table-cell" data-label="Status">
                                            <Link to={`/admin/edetails/${employer.id}`} className="el-link-wrapper">
                                        ...
                                        <td className="el-table-cell" data-label="Actions">
                                            <button
                                                className={`el-approve-button ${employer.is_approved_by_admin ? 'approved' : ''}`}
                                                onClick={() => handleApproval(employer.id, 'approve')}
                                                aria-label={`Approve employer ${employer.user_name || 'N/A'}`}
                                            >
                                                {employer.is_approved_by_admin ? 'Approved' : 'Approve'}
                                            </button>
                                            <button
                                                className={`el-reject-button ${!employer.is_approved_by_admin && employer.is_approved_by_admin !== null ? 'rejected' : ''}`}
                                                onClick={() => handleApproval(employer.id, 'reject')}
                                                aria-label={`Reject employer ${employer.user_name || 'N/A'}`}
                                            >
                                                {!employer.is_approved_by_admin && employer.is_approved_by_admin !== null ? 'Rejected' : 'Reject'}
                                            </button>
                                        </td>
                                    </tr>
                                ))}
