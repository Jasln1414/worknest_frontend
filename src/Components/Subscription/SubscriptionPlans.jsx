...
const SubscriptionPlans = () => {
  ...
  // Improved existing plan detection
  const getCurrentPlanId = () => {
    if (!usageData) return null;
    
    // Check if user has an active subscription with plan ID
    if (usageData.has_active_subscription && usageData.subscription_plan_id) {
      return usageData.subscription_plan_id;
    }
  ...
  // const handleRenewSubscription = async (planId = null) => {
  //   setLoading(true);
  //   setError(null);
  ...
  //     // If no planId provided, RENEW existing inactive subscription
  //     const subscriptionIdToRenew = renewalSubscriptionId || existingSubscriptionId;
      
  //     if (!subscriptionIdToRenew) {
  //       Swal.fire({
  //         icon: 'info',
  //         title: 'No Previous Subscription',
  //         text: 'Please select a plan to subscribe.',
  //         confirmButtonColor: '#1E3A8A',
  //       });
  //       setLoading(false);
  //       return;
  ...
  //     // Get the plan name for confirmation message
  //     const planName = usageData?.subscription_plan || 'your previous plan';
      
  //     // Confirm renewal with user
  //     const result = await Swal.fire({
  //       icon: 'question',
  //       title: `Renew ${planName}?`,
  //       text: `Do you want to renew your ${planName} subscription for another term?`,
  //       showCancelButton: true,
  //       confirmButtonText: 'Yes, Renew',
  //       cancelButtonText: 'Select New Plan',
  //       confirmButtonColor: '#1E3A8A',
  //     });
  ...
  //     // Call the RENEW endpoint specifically
  //     const response = await axios.post(
  //       `${baseURL}/payment/subscription/renew/`,
  //       { 
  //         subscription_id: subscriptionIdToRenew,
  //         plan_id: usageData?.subscription_plan_id // Include plan ID if available
  //       },
  //       { headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' } }
  //     );
      
  //     const { order_id, amount, key_id, subscription_type, plan_id } = response.data;

  //     // Load Razorpay if not available
  //     if (!window.Razorpay) {
  //       const script = document.createElement('script');
  //       script.src = 'https://checkout.razorpay.com/v1/checkout.js';
  //       script.async = true;
  //       script.onload = () => {
  //         initiatePayment(order_id, amount, key_id, plan_id, subscription_type);
  //       };
  //       script.onerror = () => {
  //         setError('Failed to load payment gateway.');
  //         setLoading(false);
  ...
  //       text: err.response?.data?.message || 'Error renewing subscription. Please try again.',
  //       confirmButtonColor: '#1E3A8A',
  //     });
  //     setError(err.response?.data?.message || 'Error renewing subscription.');
  //     setLoading(false);
...
const handleRenewSubscription = async (planId = null) => {
  ...
  try {
    ...
    if (planId) {
      // User clicked on a specific plan → create new subscription
      ...
      payload = renewalSubscriptionId
        ? { sub_id: renewalSubscriptionId }
        : existingSubscriptionId
        ? { sub_id: existingSubscriptionId }
        : usageData?.subscription_plan_id
        ? { plan_id: usageData.subscription_plan_id }
        : null;

      if (!payload) {
        setError('No subscription or plan found to renew.');
        setLoading(false);
        return;
      }
    ...
    const { order_id, amount, key_id, subscription_type, planId: renewedPlanId } = response.data;
    ...
    if (!window.Razorpay) {
      ...
      script.async = true;
      script.onload = () => {
        initiatePayment(order_id, amount, key_id, renewedPlanId || planId, subscription_type);
      };
      script.onerror = () => {
        setError('Failed to load payment gateway.');
        setLoading(false);
        ...
        {!usageLoading && usageData && (
          <div className="job-usage-dashboard">
              ...
              <div className="usage-stat-card">
                ...
                <div className="stat-content">
                  ...
                  <p className="stat-value">
                    {usageData.has_active_subscription
                      ? formatDate(usageData.subscription_end_date)
                      : `Inactive${usageData.subscription_plan ? ` (Previous: ${usageData.subscription_plan})` : ''}`}
                  </p>
                </div>
              </div>
            </div>
            ...
            {shouldShowRenewButton && (
              <div className="renewal-notice">
                <p>
                  {!usageData.has_active_subscription && usageData.subscription_plan
                    ? `Your ${usageData.subscription_plan} plan is inactive. Renew to continue using it.`
                    : isSubscriptionNearExpiry()
                    ? `Your ${usageData.subscription_plan} plan expires soon. Renew now to continue.`
                    : 'Your subscription needs attention.'}
                </p>
                <button
                  className="renew-button"
                  onClick={() => handleRenewSubscription()} // No planId for renewal
                  disabled={loading}
                >
                  {loading ? 'Processing...' : `Renew ${usageData.subscription_plan || 'Subscription'}`}
                </button>
              </div>
            )}
          </div>
        ...
        <div className="plans-grid">
          {plans.map((plan) => {
            ...
            return (
              <div
                key={plan.id}
                className={`plan-card ${isActive ? 'plan-active' : ''} ${isCurrent ? 'plan-current' : ''}`}
              >
                {isActive && <div className="current-plan-badge">CURRENT PLAN</div>}
                {isCurrent && !isActive && <div className="inactive-plan-badge">YOUR PLAN</div>}
                {plan.name.toLowerCase().includes('premium') && !isActive && !isCurrent && (
                  <div className="popular-badge">POPULAR</div>
                )}
                <h3>{plan.name}</h3>
                <p className="plan-description">{plan.description}</p>
                ...
                <button
                  onClick={() => {
                    if (isCurrent) {
                      // This is the current plan - renew it
                      handleRenewSubscription(plan.id);
                    } else if (usageData?.has_active_subscription) {
                      // Different plan but active subscription exists
                      Swal.fire({
                        icon: 'warning',
                        title: 'Active Subscription',
                        text: `You already have an active ${usageData.subscription_plan} subscription. You can switch plans after your current subscription ends on ${formatDate(usageData.subscription_end_date)}.`,
                        confirmButtonColor: '#1E3A8A',
                      });
                    } else {
                      // No active subscription - subscribe normally
                      handleSubscribe(plan.id);
                    }
                  }}
                  disabled={loading || (usageData?.has_active_subscription && !isCurrent)}
                  className={`subscribe-button ${
                    isCurrent ? 'current-plan-button' : 
                    usageData?.has_active_subscription && !isCurrent ? 'disabled-button' : 
                    'active-button'
                  }`}
                  title={
                    usageData?.has_active_subscription && !isCurrent
                      ? `Cannot subscribe until current plan expires on ${formatDate(usageData?.subscription_end_date)}`
                      : ''
                  }
                >
                  {loading ? 'Processing...' : 
                   isCurrent ? 'Renew Plan' : 
                   usageData?.has_active_subscription ? 'Subscription Active' : 
                   'Subscribe Now'}
                </button>
              </div>
            );
